...
function log.notify(scope, level, verbose, str, ...)
    if not verbose and _G.CopyPythonPath.config ~= nil and not _G.CopyPythonPath.config.debug then
        return
    end
    ...
    vim.notify(
        string.format("[copy-python-path.nvim@%s] %s", scope, string.format(str, ...)),
        level,
        { title = "copy-python-path.nvim" }
    )
end
...
function log.warn_deprecation(options)
    ...
    if uses_deprecated_option then
        ...
        log.notify(
            ...
            vim.log.levels.WARN,
            true,
            "use `:h CopyPythonPath.options` to read more."
        )
    end
end
